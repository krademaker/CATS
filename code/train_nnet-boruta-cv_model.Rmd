...
for (k in 1:cv_fold_outer) {
  ...
  for (i in 1:cv_fold_inner) {
    #Check progress
    print(paste('outer:', k, ', inner:', i, ', initialzed on:', Sys.time()))
    ...
    print(paste('neural network construction finished on', Sys.time()))
  ...
  nn_model = pred_nn(training_set, test_set, best_decay, best_size)
  ...
  model_out_path <- paste(root_dir, '/model_', k, '.rds', sep = '')
  saveRDS(nn_model$model, model_out_path)
  print(paste('saved model to:', model_out_path, 'on', Sys.time()))
}

best_accuracy_index <- which.max(unlist(final_accuracy_nn))
best_nn_model <- model_list[best_accuracy_index]
