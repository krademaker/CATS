...
#Create a obeject with the self organising map components
...
#Dendrogram
call.dendro <- as.dendrogram(hclust(d=dist(x=train_call_clinical)))
...
Then for the feature selection with cross-validation
...
#rownames(train_all) <- train_all$Row.names
#colnames(train_all)[1] <- "SampleID"
#train_all2 <- as.data.frame(train_all[,2:2836])

#Correlation Matrix
correlationmatrix <- cor(train_call_clinical)
highlycorrelated <- 
findCorrelation(correlationmatrix, cutoff = 0.75)

new.train_call_clinical <- train_call_clinical[,-highlycorrelated]

#ROC Variance Importance
rocVarImp <- filterVarImp(as.data.frame(train_call_clinical), train_all_sorted$Subgroup)

# ensure results are repeatable
set.seed(7)

call.new.train <- as.data.frame(call.new)
colnames(call.new.train)[2835] <- "Subgroup"
# prepare training scheme
control <- trainControl(method="repeatedcv", number=30, repeats=3)
# train the model 
model <- train(Subgroup~., data=call.new.train, method="lvq", preProcess = "scale", trControl=control)
