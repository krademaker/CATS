...
pred_nn <- function(training_set, test_set, decay_model, size_model){
  #CV
  ...
  if (decay == "" | size == ""){
    ...
    nnetGrid <- expand.grid(decay = decay_model, size = size_model)
  }
  ...
  #Features out otherwise statistical doesnt work
  ...
  return(list('predictions'=predictions.nn, 'test'=test, 'model'=n, 'size' = nnetGrid$size, 'decay' = nnetGrid$decay))
}
#####Start program
...
for (k in 1:cv_fold_outer) {
  ...
  nn_model = pred_nn(training_set, test_set, hyperparameters_list$decay[[top_index]], hyperparameters_list[[top_index]])
